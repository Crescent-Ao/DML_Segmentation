...
class MSDataSet(Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        self.augtransform_train = A.Compose(
            [
                A.RandomCrop(height=cfg.train_sr.height, width=cfg.train_sr.width, p=0.7),
        ...
        self.augtransform_test = A.Compose([A.Resize(height=cfg.test_sr.height, width=cfg.test_sr.width, p=1)])
    ...
    @staticmethod
    def multi_scale_label(img, mask):
        # 多尺度测试的方式
        fy_scale = 0.5 + random.randint(0, 15) / 10.0
        fx_scale = 0.5 + random.randint(0, 10) / 10.0
        rgb_img = cv2.resize(rgb_img, None, fx=fx_scale, fy=fy_scale, interpolation=cv2.INTER_LINEAR)
        infrared_img = cv2.resize(infrared_img, None, fx=fx_scale, fy=fy_scale, interpolation=cv2.INTER_LINEAR)
        mask = cv2.resize(mask, None, fx=fx_scale, fy=fy_scale, interpolation=cv2.INTER_NEAREST)
