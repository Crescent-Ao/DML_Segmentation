...
def intersect_and_union(num_class, logits, labels):
    ...
    area_intersect = torch.histc(
        intersect.float(), bins=(num_class), min=0, max=num_class - 1)
    area_pred_label = torch.histc(
        logits.float(), bins=(num_class), min=0, max=num_class - 1)
    area_label = torch.histc(
        labels.float(), bins=(num_class), min=0, max=num_class - 1)
